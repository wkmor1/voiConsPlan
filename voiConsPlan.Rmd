...
In Box 2 we demonstrated how to calculate EVPI for a two site conservation plan with continuous uncertainty. Here we extend that problem to 25 sites and two species to demonstrate how the problem can be solved using Zonation with random (Monte Carlo) bootstrap samples (see section "the boostrap" for more detail) of species distributions. The same method we apply here can be used for much larger numbers of sites and species and is only limited by computing resources. Here we demonstrate the method using only two bootstrapped samples in practice (as in the case study that follows) a far larger number samples would be neccessary.
