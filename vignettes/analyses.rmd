```{r knitr_setup, echo = FALSE, message = FALSE, warning = FALSE, results = "hide"}
knitr::opts_chunk$set(
  cache = TRUE, echo = FALSE, message = FALSE, warning = FALSE, results = "hide"
)
```

```{r analyses_setup, cache = FALSE}
library(ALA4R)
library(doSNOW)
library(dplyr)
...
models_boot <-
  foreach(
    bs_sample = bs_samples,
    .packages = c("dplyr", "voiConsPlan")
  ) %dopar% {
    lapply(
      spp_of_interest, fit_maxent_model, bs_sample, background, predictors
    ) %>%
    lapply(project_maxent, predictors) %>%
    brick
  }
...
plans_boot <-
  foreach(
    models = models_boot,
    .packages = c("dplyr", "rzonation")
  ) %dopar% {
    zonation(models, settings = zonation_settings) %>%
    get_rank
  }
```
