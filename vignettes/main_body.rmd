...
First we calculated the EVWOI, that is, how well do we expect the conservation plan to perform (on average) when basing it only on the 30 observations per species. To calculate EVWOI we first averaged the bootstrap species distributions to produce one map per species. Using these six maps (Figure \ref{fig:plot_models}) we applied the Zonation algorithm to rank the grid cells of the Hunter region in order of priority. Figure \ref{fig:plot_plan} shows the grid cells of the Hunter ranked in their order of priority. According to Zonation, the grid cells in the top 20% are located in the hinterland of the south-east and central coast. Protecting this region of the Hunter, on average, protects `r fmt(evwoi * 100, 1)`% of the total carrying capacity of the six species of interest. This value is the EVWOI.
