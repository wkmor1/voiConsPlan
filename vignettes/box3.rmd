...
In Box 2 we demonstrated how to calculate EVPI for two site conservation plan with continuous uncertainty.
Here we extend that problem to 25 sites and two species demonstrate how the problem can be solved using Zonation on random bootstrap samples (Monte Carlo sample) of species distributions. The same method we apply here can be used for much larger numbers of sites and species and is only limited by computing resources.
